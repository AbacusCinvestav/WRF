...
module load netCDF/parallel/1.7.0 netCDF/c/4.4.1-rc2-TEST netCDF/fortran/4.4.3-TEST

echo -e '\E[37;44m'"\033[1mDescargando WRFV3.9.1.1\033[0m"

wget -c http://www2.mmm.ucar.edu/wrf/src/WRFV3.9.1.1.TAR.gz

tar -xzvf WRFV3.9.1.1.TAR.gz

rm -rf WRFV3.9.1.1.TAR.gz

cd WRFV3

./clean -a 

cat <<'EOF' > configure.wrf
# configure.wrf
#
# Original configure options used:
# ./configure 
# Compiler choice: 16 #16. (dm+sm)   INTEL (ifort/icc)
# Nesting option: 1
#
...
# Settings for    Linux x86_64 ppc64le i486 i586 i686, ifort compiler with icc  (dm+sm)
#
...
CC_TOOLS_CFLAGS = -DNMM_CORE=$(WRF_NMM_CORE)
 ...
 LIB_EXTERNAL    = \
                      -L$(WRF_SRC_ROOT_DIR)/external/io_netcdf -lwrfio_nf -L/lustre/apps/netcdf/c-4.4.1-rc2-fortran-4.4.3-TEST/lib -lnetcdff -lnetcdf  -L$(WRF_SRC_ROOT_DIR)/external/io_pnetcdf -lwrfio_pnf -L/lustre/apps/netcdf/parallel-netcdf/lib -lpnetcdf   -L/lustre/apps/HDF5/1.8.17/parallel2/lib -lhdf5_fortran -lhdf5 -lhdf5_hl -lm -lz
...
NETCDFPATH      =    /lustre/apps/netcdf/c-4.4.1-rc2-fortran-4.4.3-TEST/
HDF5PATH        =    /lustre/apps/HDF5/1.8.17/parallel2//
WRFPLUSPATH     =    
RTTOVPATH       =    
PNETCDFPATH     =    /lustre/apps/netcdf/parallel-netcdf/
...
.F.i:
	...
	$(CPP) -I$(WRF_SRC_ROOT_DIR)/inc $(CPPFLAGS) $*.G > $*.H
	sed -e "s/gridmu/grid%mu/g" -e "s/gridMu/grid%Mu/g" $*.H > $*.i
...
.F.f90:
	...
	sed -e "s/grid%mu/gridmu/g" -e "s/grid%Mu/gridMu/g" -e "s/^\!.*'.*//" -e "s/^ *\!.*'.*//" $*.F > $*.G
	$(SED_FTN) $*.G > $*.H 
	$(CPP) -I$(WRF_SRC_ROOT_DIR)/inc $(CPPFLAGS) $*.H  > $*.I
	sed -e "s/gridmu/grid%mu/g" -e "s/gridMu/grid%Mu/g" $*.I > $@
	$(RM) $*.G $*.H $*.I
...
echo -e '\E[37;44m'"\033[1mEsperando 3 segundos para iniciar compilacion de WRF\033[0m"

sleep 3

./compile em_real 3>&2 2>&1 | tee ../WRF.log

echo -e '\E[37;44m'"\033[1m Compilacion de WRFv3.9.1.1 terminada, los logs se guardaron en WRF.log \033[0m"

cd ../.

echo 

echo -e '\E[37;44m'"\033[1m Descargando WPSv3.9.1.1 \033[0m"

wget -c http://www2.mmm.ucar.edu/wrf/src/WPSV3.9.1.TAR.gz

tar -xzvf WPSV3.9.1.TAR.gz

rm -rf WPSV3.9.1.TAR.gz

cd WPS

cat <<'EOF' > configure.wps
# configure.wps
#
# This file was automatically generated by the configure script in the
# top level directory. You may make changes to the settings in this
...
WRF_DIR			=	/tmp/WRF/WRFV3

NETCDF			=	/lustre/apps/netcdf/c-4.4.1-rc2-fortran-4.4.3-TEST/
...
WRF_LIB         =       -L$(WRF_DIR)/external/io_grib1 -lio_grib1 \
                        ...
                        -L$(NETCDF)/lib -lnetcdff -lnetcdf -liomp5 -lpthread -lhdf5_hl
...
#
#
COMPRESSION_LIBS    = -L/lustre/apps/jasper/jasper-libpng/lib/ -ljasper -lpng -lz
COMPRESSION_INC     = -I/lustre/apps/jasper/jasper-libpng/include/
FDEFS               = -DUSE_JPEG2000 -DUSE_PNG
SFC                 = ifort
SCC                 = icc
DM_FC               = mpiifort
...
echo -e '\E[37;44m'"\033[1mEsperando 3 segundos para iniciar compilacion de WPS\033[0m"

./compile 3>&2 2>&1 | tee ../WPS.log

echo -e '\E[37;44m'"\033[1m Compilacion de WRFv3.9.1.1 terminada, los logs se guardaron en WPS.log \033[0m"

echo 

echo -e '\E[37;44m'"\033[1m Fin de la Instalacion\033[0m"
